...
const vector<string> labels = {"None", "Tire"};
...
void YOLOV8::preprocess_img(Mat &frame) {
    ...
    try {
        ...
        this->width_ratio = (float)frame.cols / (float)(resize.resized_image.cols - resize.dw);
        this->height_ratio = (float)frame.rows / (float)(resize.resized_image.rows - resize.dh);
...
void YOLOV8::postprocess_img(Mat &frame, float *detections, ov::Shape &output_shape)
{
    ...
    for (int i = 0; i < output.size(); i++)
    {
        ...
        box.x = this->width_ratio * box.x;
        box.y = this->height_ratio * box.y;
        box.width = this->width_ratio * box.width;
        box.height = this->height_ratio * box.height;
        ...
        // Detection box text
        std::string classString = labels[classId] + ' ' + std::to_string(confidence).substr(0, 4);
